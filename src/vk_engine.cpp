...
void VulkanEngine::init()
{
    ...
    SdlUtil::sdl_set_cursor_game(_window);
}
...
void VulkanEngine::init_default_data() {
	...
	for (int x = 0; x < 16; x++) {
		for (int y = 0; y < 16; y++) {
			pixels[y * 16 + x] = ((x % 2) ^ (y % 2)) ? magenta : magenta;
		}
	}
...
void VulkanEngine::draw_main(VkCommandBuffer cmd) // TODO :: Add a color image to fix texture and compute isssues with MSAA
{
	...
	//draw the triangle
	VkRenderingAttachmentInfo colorAttachment = vkinit::attachment_info(_drawImage.imageView, nullptr, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, _colorImage.imageView);
...
{
	VkRenderingAttachmentInfo colorAttachment = vkinit::attachment_info(targetImageView, nullptr, VK_IMAGE_LAYOUT_GENERAL, _colorImage.imageView);
...
void VulkanEngine::draw() // TODO:: possibly change how transition images from color to draw to depth
{
...
void VulkanEngine::run()
{
    ...
    while (!bQuit) {
        ...
        while (SDL_PollEvent(&e) != 0) {
            // close the window when user alt-f4s or clicks the X button or presses escape
            ...
            // this code block switches the cursor from being locked and hiiden to window to opposite
            if(e.type == SDL_KEYDOWN){
                if (!e.key.repeat && e.key.keysym.sym == SDLK_TAB) {

                    if(SdlUtil::sdl_cursor_state){
                        SdlUtil::sdl_set_cursor_free(_window);
                    }
                    else{
                        SdlUtil::sdl_set_cursor_game(_window);
                    }
                ...
                if (!e.key.repeat && e.key.keysym.sym == SDLK_i) {
                    if(renderScale >= 0.2f){
                        renderScale -= 0.05f;
                    }
                }
                if (!e.key.repeat && e.key.keysym.sym == SDLK_o) {
            ...
            //handle main camera inputs and imgui inputs
...
void VulkanEngine::update_scene()
{
    ...
    // Invert the Y direction on projection matrix to align with OpenGL/Gltf axis
    projection[1][1] *= -1;
    ...
    glm::mat4 translation = glm::translate(glm::vec3{ 0.1f, 0.1f, 0.1f }); // Move right
    glm::mat4 scale = glm::scale(glm::vec3{ 1.0f }); // Scale by 1000
    glm::mat4 scale1 = glm::scale(glm::vec3{ 5.0f }); // Scale by 1000

    loadedScenes["structure"]->Draw(translation*scale, drawCommands);

    //auto cone = loadedScenes["object"]->nodes.find("Cone");
    //auto& coneNode = cone->second;
    //coneNode->localTransform = translation*scale; // Apply scale then translatio
    //coneNode->refreshTransform(translation*scale);  
    //loadedScenes["object"]->Draw(glm::mat4{ 1.f }, drawCommands);
    //loadedScenes["object"]->print_nodes();
}
...
AllocatedImage VulkanEngine::create_image(VkExtent3D size, VkFormat format, VkImageUsageFlags usage, bool mipmapped)
{
    ...
    // if the format is a depth format, we will need to have it use the corrfect
    // aspect flag
...
void VulkanEngine::init_vulkan()
{
    ...
    auto inst_ret = builder.set_app_name("Pouch Engine")
...
void VulkanEngine::init_swapchain() // TODO:: maybe redo ur draw image and scond color image for MSAA
{
	...
	colorImageUsages |= VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT;
	colorImageUsages |= VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT;

	VkImageCreateInfo cimg_info = vkinit::image_create_info(_colorImage.imageFormat, colorImageUsages, drawImageExtent);
	...
	VkImageViewCreateInfo cview_info = vkinit::imageview_create_info(_colorImage.imageFormat, _colorImage.image, VK_IMAGE_ASPECT_COLOR_BIT);

	VK_CHECK(vkCreateImageView(_device, &cview_info, nullptr, &_colorImage.imageView));
	...
	_mainDeletionQueue.push_function([=]() {
        vkDestroyImageView(_device, _colorImage.imageView, nullptr);
...
{
    std::string structurePath = { "..\\..\\assets\\wrath_of_the_dragon.glb" }; // path for our scene file
    ...
    std::string objectPath = { "..\\..\\assets\\fours.glb" };
    auto objectFile = loadGltf(this,objectPath);

    assert(objectFile.has_value()); // check if the file is populated

    loadedScenes["object"] = *objectFile;
}
...
void GLTFMetallic_Roughness::build_pipelines(VulkanEngine* engine) // TODO:: Fix for MSAA add the second color attachment color image
{
